...
class RayRPGTrainer(RayPPOTrainer):
    """
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    if self.config.algorithm.use_kl_in_reward:
                        ...
                        if self.config.actor_rollout_ref.actor.use_iterative_ref_model == False:
                            with _timer('ref', timing_raw):
                                ref_log_prob = self.ref_policy_wg.compute_ref_log_prob(new_batch)
                                new_batch = new_batch.union(ref_log_prob)
                                new_batch.batch['ref_log_prob'] = ref_log_prob.batch['ref_log_prob']
                        else: 
                            with _timer('ref', timing_raw):
                                ref_log_prob = old_log_prob
                                new_batch = new_batch.union(ref_log_prob)
                                new_batch.batch['ref_log_prob'] = ref_log_prob.batch['old_log_probs']
                    ...
                    with _timer('reward', timing_raw):
                        ...
                        if self.config.algorithm.use_kl_in_reward:
                            ...
                            clip_ratio_low = self.config.actor_rollout_ref.actor.clip_ratio_low if self.config.actor_rollout_ref.actor.clip_ratio_low is not None else clip_ratio
                            clip_ratio_high = self.config.actor_rollout_ref.actor.clip_ratio_high if self.config.actor_rollout_ref.actor.clip_ratio_high is not None else clip_ratio
                            clip_ratio_c = self.config.actor_rollout_ref.actor.get('clip_ratio_c', 3.0)
                            clip_config = {"cliprange": clip_ratio,
                    ...
                    if self.use_reference_policy or self.config.algorithm.use_kl_in_reward or self.config.actor_rollout_ref.actor.use_kl_loss:
                        if self.config.actor_rollout_ref.actor.use_iterative_ref_model == False:
                            ...
                            with _timer('ref', timing_raw):
                                ref_log_prob = old_log_prob
                                batch = batch.union(ref_log_prob)
                                batch.batch['ref_log_prob'] = ref_log_prob.batch['old_log_probs']
