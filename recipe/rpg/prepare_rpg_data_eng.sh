...
export TRAIN_FILE=${TRAIN_FILE:-"${VERL_HOME}/data/dapo-math-17k-eng.parquet"}
...
if [ ! -f "${TRAIN_FILE}" ] || [ "${OVERWRITE}" -eq 1 ]; then
  wget -O "${TRAIN_FILE}" "https://hf-mirror.com/datasets/math-dataset/DAPO-17k-Eng/resolve/main/dapo-math-17k-eng.parquet?download=true"
fi
