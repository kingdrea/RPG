...
from verl.trainer.ppo.core_algos import compute_policy_loss, compute_policy_loss_reinforce, kl_penalty, agg_loss
...
class DataParallelPPOActor(BasePPOActor):
    ...
    def compute_log_prob(self, data: DataProto, calculate_entropy=False) -> torch.Tensor:
        ...
        # if self.config.optim.get('optim_type', 'adamw') == "schedulefree":
        try:
            self.actor_optimizer.eval()
        except:
            pass
        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    if self.config.use_kl_loss or self.config.use_kl_in_reward:
                        if self.config.use_iterative_ref_model:
                            ref_log_prob = data['old_log_probs']
                            data['ref_log_prob'] = old_log_prob
                        else:
                            ref_log_prob = data.get('ref_log_prob')
