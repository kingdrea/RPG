...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
        ...
        if role == 'actor' and optim_config is not None:
            ...
            optim_type = optim_config.get('optim_type', 'adamw')
            if optim_type == 'adamw':
                actor_optimizer = optim.AdamW(actor_module_fsdp.parameters(),
                                          lr=optim_config.lr,
                                          betas=optim_config.get('betas', (0.9, 0.999)),
                                          weight_decay=optim_config.get('weight_decay', 1e-2))
            elif optim_type == 'schedulefree':
                # try to import schedulefree, if not found, use adamw
                try:
                    import schedulefree
                    actor_optimizer = schedulefree.AdamWScheduleFree(actor_module_fsdp.parameters(),
                                          lr=optim_config.lr,
                                          betas=optim_config.get('betas', (0.9, 0.999)),
                                          weight_decay=optim_config.get('weight_decay', 1e-2))
                    # actor_optimizer.train()
                except ImportError:
                    print('schedulefree not found, use adamw instead')
                    actor_optimizer = optim.AdamW(actor_module_fsdp.parameters(),
                                          lr=optim_config.lr,
                                          betas=optim_config.get('betas', (0.9, 0.999)),
                                          weight_decay=optim_config.get('weight_decay', 1e-2))
...
class CriticWorker(Worker):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        optim_type = config.optim.get('optim_type', 'adamw')
        if optim_type == 'adamw':
            ...
            try:
                import schedulefree
                critic_optimizer = schedulefree.AdamWScheduleFree(critic_module.parameters(),
                                               lr=config.optim.lr,
                                               betas=config.optim.get('betas', (0.9, 0.999)),
                                               weight_decay=config.optim.get('weight_decay', 1e-2))
